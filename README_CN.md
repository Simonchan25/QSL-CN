...
QSL-CN 是一个用于观察和分析A股市场的个人学习项目。通过 Tushare API 获取实时与历史行情数据，进行数据清洗与多维度因子评分（技术面+情绪面+基本面），结合本地部署的大模型（Qwen2.5 via Ollama）进行市场解读。系统可自动生成每日早报，包含热点分析、标的推荐、图表展示，帮助理解市场轮动规律和情绪节奏。
...
### 3. 专业研报
- **早报生成**：自动生成每日早报，包含7大板块（盘前热点、公告精选、全球市场、连板分析、机构动向、新高榜单、人气排行）
- **全市场综合分析**：覆盖全A股5400+只股票的深度量化分析
...
- **Tushare Pro API (5000积分权限)**：
  - 实时与历史行情数据（日线、基本面、技术指标）
  - 多源新闻聚合（新浪、华尔街见闻、同花顺、东方财富等）
  - 机构交易数据、涨跌停统计
  - 概念板块数据和北向资金流向
  - 公告和重大新闻
